...
static uint32_t bg_res(void) {
  if (s_scene == SCENE_DOWNTOWN) {
    ...
    if (s_theme == THEME_MONO)   return RESOURCE_ID_IMAGE_MAP_DOWNTOWN_MONO;
    return RESOURCE_ID_IMAGE_MAP_DOWNTOWN;
  }
  if (s_scene == SCENE_RESIDENTIAL) {
    if (s_theme == THEME_EPAPER) return RESOURCE_ID_IMAGE_MAP_RESIDENTIAL_EPAPER;
...
static void apply_theme(void) {
  ...
  s_icon_heart_bmp = gbitmap_create_with_resource(
    s_theme == THEME_MONO ? RESOURCE_ID_IMAGE_HUD_ICON_HEART_MONO : RESOURCE_ID_IMAGE_HUD_ICON_HEART);
  ...
  s_bar_heart_bmp = gbitmap_create_with_resource(
    s_theme == THEME_MONO ? RESOURCE_ID_IMAGE_HUD_BAR_HEART_MONO : RESOURCE_ID_IMAGE_HUD_BAR_HEART);
